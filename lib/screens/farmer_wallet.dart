...
class FarmerWallet extends StatefulWidget {
  ...
  const FarmerWallet({super.key, required this.userId});
...
class FarmerWalletState extends State<FarmerWallet> {
  ...
  Future<void> fetchProducts() async {
    final url = Uri.parse("https://flutter.zuasoko.com/get_farmer_products.php");
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          for (var product in data) {
            double pricePerKg = double.tryParse(product["price_per_kg"].toString()) ?? 0.0;
  ...
  Future<void> cashOut() async {
    final url = Uri.parse("https://flutter.zuasoko.com/cashout.php");
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(responseData['message'] ?? "Cashout successful")),
        );
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Farmer Wallet")),
      body: isLoading
                    ...
                    Expanded(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          // double pricePerKg = double.tryParse(product["price_per_kg"].toString()) ?? 0.0;
                          // double weight = double.tryParse(product["weight"].toString()) ?? 0.0;
                          // double itemTotal = pricePerKg * weight;
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              leading: ClipRRect(
                                ...
                                child: Image.network(
                                  "https://flutter.zuasoko.com/product_images/${product['image']}",
                                  width: 50,
                                  height: 50,
                                  fit: BoxFit.cover,
                                  errorBuilder: (context, error, stackTrace) {
                                    print("Failed to load image: ${product['image']}, Error: $error");
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  Text("Price per kg: \$${double.tryParse(product["price_per_kg"]?.toString() ?? "0")?.toStringAsFixed(2) ?? "0.00"}"),
                                  Text("Weight: ${double.tryParse(product["weight"]?.toString() ?? "0")?.toStringAsFixed(2) ?? "0.00"} kg"),
                                  Text(
                              ...
                              trailing: Text(
                                ...
                                style: TextStyle(
                                  color: product["status"] == "accepted" ? Colors.green : Colors.red,
